...
- `pi_replicated_snapshot_support` - (Optional, String) Indicates if the pvm instance should be created on a storage pool that supports replicated instance snapshots. Allowable values: `none`, `zonal_only`, `zonal_with_regional`.
