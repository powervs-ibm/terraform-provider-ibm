...
- `pi_replicated_snapshot_support` - (Optional, String, Forces new resource) Indicates if the image is required to be created in a storage pool that supports replicated instance snapshots. Allowable values: `none`, `zonal_only`, `zonal_with_regional`.
