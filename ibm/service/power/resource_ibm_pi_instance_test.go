...
func TestAccIBMPIInstanceVirtualSerialNumber(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMPIInstanceVirtualSerialNumber(name, power.OK, "VMVirtualSerialNumber", "s922"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMPIInstanceExists(instanceRes),
					resource.TestCheckResourceAttr(instanceRes, "pi_instance_name", name),
...
func testAccCheckIBMPIInstanceVirtualSerialNumber(name, instanceHealthStatus, epic, systype string) string {
	return fmt.Sprintf(`
	resource "ibm_pi_key" "key" {
		pi_cloud_instance_id = "%[1]s"
		pi_key_name          = "%[2]s"
